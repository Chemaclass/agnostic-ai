...
func TestImportFromClaude_StripsProvenanceHeaderFromMainFile(t *testing.T) {
	dir := t.TempDir()
	body := "# Project\n\nTop-level instructions.\n"
	// A synced CLAUDE.md carries the provenance header on line 1.
	writeFile(t, filepath.Join(dir, "CLAUDE.md"), header.With(body, header.FormatMarkdown))
	if err := importFromClaude(dir, rootSources(), defaultClaudeLayout()); err != nil {
		t.Fatal(err)
	}
	got, err := os.ReadFile(filepath.Join(dir, agnosticMainFile))
	...
	if strings.Contains(string(got), "Generated by agnostic-ai") {
		t.Errorf("provenance header leaked into %s: %s", agnosticMainFile, got)
	}
	if string(got) != body {
		t.Errorf("seed not byte-identical to header-stripped body.\nwant:\n%s\ngot:\n%s", body, got)
	}
}
...
func TestImportFromClaude_HonorsCustomSourceDirs(t *testing.T) {
	...
	if err := importFromClaude(dir, src, defaultClaudeLayout()); err != nil {
		t.Fatal(err)
	}
