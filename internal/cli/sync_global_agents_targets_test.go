...
func TestSyncGlobal_CheckAcceptsOlderStateVersion(t *testing.T) {
	...
	older := strings.Replace(string(data), `"version": 3`, `"version": 2`, 1)
	if older == string(data) {
		t.Fatalf("state has no version 3 marker:\n%s", data)
	}
	mustWriteGlobalTest(t, statePath, older)
	if _, _, err := runGlobalAgentTest("--check"); err != nil {
		t.Errorf("version 2 state with the same ownership reported drift: %v", err)
	}
}
