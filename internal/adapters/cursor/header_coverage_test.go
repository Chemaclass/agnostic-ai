...
// TestEmit_ProvenanceHeaderOnEveryEmittedFile is the cursor adapter's
// header-coverage contract: every .mdc and .md file the adapter writes
// must carry the agnostic-ai provenance marker. .cursor/mcp.json is
// JSON-exempt.
	...
	entries := []spec.Entry{
		...
		},
		{
			...
			Meta: map[string]any{
				...
				"auth": map[string]any{"CLIENT_ID": "abc123", "scopes": []any{"read"}},
			},
		},
		{
