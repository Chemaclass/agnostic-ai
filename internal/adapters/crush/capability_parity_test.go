...
// invariant that the crush adapter actually emits something for every
...
// Crush exclusively through the shared AGENTS.md entry-point that
// `sync` writes centrally (see crush.go), a write this per-package
// test never observes because it calls Adapter.Emit directly. Declaring
// KindRule keeps the "unsupported" warning honest (rules do reach
// Crush) without this adapter ever touching a rules file itself.
...
// every kind crush does not declare in caps.Supports (Agent, Command,
// Settings). Hook is no longer in this list: it is declared in
// caps.Supports as of #629, and its own coverage gap (a non-PreToolUse
// event) is exercised separately in crush_test.go, not through this
// capability-warning path. A future caps.Supports expansion needs to
