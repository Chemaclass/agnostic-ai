...
// A WARP.md without our provenance marker is user-authored and left
// alone. Warp reads WARP.md ahead of AGENTS.md when both exist
// (docs.warp.dev/agents/capabilities/rules), so leaving it untouched
// also means every rule sync just wrote to AGENTS.md never reaches
// Warp; the adapter must say so.
func TestEmit_KeepsLegacyWarpMd_WhenUserAuthored(t *testing.T) {
	...
	warning := buf.String()
	for _, want := range []string{"warp:", "WARP.md", "AGENTS.md"} {
		if !strings.Contains(warning, want) {
			t.Errorf("expected a shadow warning naming %q, got: %q", want, warning)
		}
	}
}

// A previously agnostic-generated WARP.md gets migrated away by
// MigrateLegacyFile before the shadow check runs, so no warning fires:
// there is nothing left on disk to shadow AGENTS.md.
func TestEmit_NoShadowWarning_WhenLegacyWarpMdMigrated(t *testing.T) {
	dir := testutil.TempCwd(t)
	buf := swapNoteWarner(t)
	...
	// The rename notice from MigrateLegacyFile is expected here; only the
	// shadow-specific wording must be absent.
	if warning := buf.String(); strings.Contains(warning, "takes priority") {
		t.Errorf("expected no shadow warning once the legacy file is migrated away, got: %q", warning)
	}
}

// No WARP.md at all: nothing to shadow AGENTS.md, so no warning fires.
func TestEmit_NoShadowWarning_WhenNoLegacyWarpMd(t *testing.T) {
	testutil.TempCwd(t)
	buf := swapNoteWarner(t)
	...
	if warning := buf.String(); warning != "" {
		t.Errorf("expected no shadow warning without a legacy WARP.md, got: %q", warning)
	}
}
