...
func TestEmit_NoAGENTSMd_ByDefault(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".agents/AGENTS.md")); !os.IsNotExist(err) {
		t.Errorf("adapter should not write AGENTS.md by default; sync owns the entry-point, err=%v", err)
	}
}

// A pre-existing `.agent/AGENTS.md` with our provenance marker is the
// pre-#1114 entry-point default. Emit renames it to
// `.agent/AGENTS.md.bak` so the user notices the move to the documented
// `.agents/AGENTS.md` path.
func TestEmit_MigratesLegacyEntryPoint_WhenAgnosticGenerated(t *testing.T) {
	dir := testutil.TempCwd(t)

	legacy := "# AI Project Conventions\n\nGenerated by agnostic-ai. Do not edit by hand.\n\nstale.\n"
	if err := os.MkdirAll(filepath.Join(dir, ".agent"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(dir, ".agent", "AGENTS.md"), []byte(legacy), 0o644); err != nil {
		t.Fatal(err)
	}

	entries := []spec.Entry{{Kind: spec.KindRule, Name: "r", Body: "new rule"}}
	if err := New().Emit(emit.NewSession(), spec.NewBundle(entries), &config.Config{}, false); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(filepath.Join(dir, ".agent", "AGENTS.md")); !os.IsNotExist(err) {
		t.Errorf("legacy .agent/AGENTS.md should be removed, err=%v", err)
	}
	bak := readFile(t, filepath.Join(dir, ".agent", "AGENTS.md.bak"))
	if !strings.Contains(bak, "stale.") {
		t.Errorf("backup should preserve old content: %s", bak)
	}
}

// A `.agent/AGENTS.md` not authored by agnostic-ai is left alone.
func TestEmit_KeepsLegacyEntryPoint_WhenUserAuthored(t *testing.T) {
	dir := testutil.TempCwd(t)

	userContent := "# My own notes\n"
	if err := os.MkdirAll(filepath.Join(dir, ".agent"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(dir, ".agent", "AGENTS.md"), []byte(userContent), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if got != userContent {
		t.Errorf("user-authored .agent/AGENTS.md should be preserved verbatim: %q", got)
	}
