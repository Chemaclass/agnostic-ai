...
// one silently would be the bigger surprise; docs/site/content/docs/targets/_index.md
...
func emitLegacyRulesFile(sess *emit.Session, b spec.Bundle, cfg *config.Config, dryRun bool) error {
	...
	// With no always-on rule, only a local block keeps the file: the
	// central writer skips an entry point this document owns.
	if len(alwaysOn) == 0 && content == sb.String() {
		return nil
	}
