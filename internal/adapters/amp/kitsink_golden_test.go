...
// an amp sync over the canonical kit-sink bundle (3 agents + 3
// skills + 3 rules + 3 MCPs) with rules-file and emit-skills-as-
// commands both opted in so every supported kind lands at an
// observable path.
//
// Diff regressions (frontmatter key reorder, MCP transport rendering
// drift, JSON merge churn) trip this test and must be acknowledged
// by either fixing the adapter or regenerating the snapshot:
//
//	UPDATE_GOLDEN=1 go test ./internal/adapters/amp/ -run TestKitSink_GoldenSnapshot
