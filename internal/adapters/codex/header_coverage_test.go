...
// TestEmit_ProvenanceHeaderOnEveryEmittedFile is the codex adapter's
...
// resulting file. The only exception is the project-root AGENTS.md,
// which the sync layer writes (this adapter never touches it). JSON
// outputs (.codex/hooks.json today) also legitimately skip the header
// because JSON has no comment syntax.
//
// The kit-sink bundle covers agents, skills, rules, hooks, commands,
// and MCPs so any future emit path forgetting WithHeader / HeaderBlock
// gets caught here.
func TestEmit_ProvenanceHeaderOnEveryEmittedFile(t *testing.T) {
	...
	err := filepath.WalkDir(dir, func(path string, d fs.DirEntry, walkErr error) error {
		...
		if jsonExempt(rel) {
			// Hooks land in JSON; the format intentionally has no
			// header. Make sure the file is at least valid (non-empty)
			// so a regression that produces an empty file still trips.
			info, err := d.Info()
			if err != nil {
				return err
			}
...
func kitSinkBundle() spec.Bundle {
	entries := []spec.Entry{
		...
		},
		{
			...
			Meta: map[string]any{"url": "https://example.test/mcp"},
		},
		{
...
// headFor returns the first line of data (or up to 120 bytes) for
