...
var registry = map[Code]Entry{
	...
	CodeIgnoreOverwrite: {
		...
		Cause: "A target's ignore file (.cursorignore, .geminiignore, .aiderignore, .devinignore, .windsurfignore, .kiroignore, .trae/.ignore, .aiignore) carries no agnostic-ai header and holds patterns the ignore specs do not. These files keep credentials out of agent context, so sync writes nothing rather than dropping them.",
