...
- **JetBrains plugin** (#101): first-party plugin at `editors/jetbrains/` (v0.1.0). IntelliJ Platform 2024.2+. Schema-backed editing for `agnostic.config.yaml`, Tools menu entries (`Sync`, `Sync — check`, `Doctor — auto-fix`, `Status`), editor banner above each spec with one `Render to <target>` link per configured target, and a status bar widget polling `sync --check --json` for the drift count.
...
- **Zed Tasks for hooks** (#109): when `outputs.zed.tasks-file` is set, every hook spec emits as a [Zed Task](https://zed.dev/docs/tasks) (`sh -c "<command>"` with the hook name as the label) into that JSON file, runnable on demand from Zed's command palette. Hooks now count as natively supported by Zed in the `validate` orphan-kind check.
- **Warp Workflows** (#110): when `outputs.warp.workflows-dir` is set, each agent emits as a [Warp Workflow](https://docs.warp.dev/features/warp-drive/workflows) YAML (`<dir>/<name>.yaml` with `name`/`command`/`description`/`tags`). The `## Agents` section in `AGENTS.md` simultaneously downgrades to reference pointers so each agent body lives in exactly one place.
