...
### Fixed
- `.codex/config.toml` cleaned up when sync renders no content for it.
- Codex sweeps legacy `.agents/agents/` and `.agents/skills/` trees on sync.
- `import codex` captures MCP `description`, `disabled`, and `roots` fields.
...
- Gemini command TOMLs now carry the `# Generated by agnostic-ai` provenance header.
...
- Shared `sliceMainFileByH2` importer unwraps `## Rules` / `## Agents` / `## Skills` H2 wrappers into per-H3 spec files and strips `Generated by agnostic-ai.` preamble + `<!-- source: ... -->` comments so legacy concatenated rules-file layouts (aider CONVENTIONS.md, amp / warp / zed / opencode / copilot AGENTS.md) reach a byte-equal fixed point on round-trip.
