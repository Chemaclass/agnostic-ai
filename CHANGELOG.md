...
- `TestWatchSync_IncrementalReSyncsOnlyAffectedTarget` no longer flakes on Windows CI (#585). The test edited a spec as soon as the initial sync's output file appeared, but the watcher takes its baseline mtime snapshot after that sync, so on a slow runner the edit landed before the baseline existed. The baseline then already contained it, no change was ever detected, and the test failed only when its own 15s deadline expired. It now waits for the watcher's banner, which prints immediately after the snapshot. All four watch tests also set mtimes explicitly with `os.Chtimes` rather than sleeping to advance them.
