...
### Added
- Track `.agnostic-ai/AGNOSTIC_AI.md` in git. It is the canonical entry-point body that drives every per-target file.
- `sync` records every emitted path in a ledger inside `.agnostic-ai/.sync-state` (schema `version: 2`). Next sync removes any path the prior run wrote but the current run did not. Hand-authored files (no provenance header) are preserved. Empty parent dirs pruned. Older state files without `outputs` skip the sweep on first run.

### Changed
- Provenance header now reads `Generated by agnostic-ai. Do not edit this file directly; edit specs under .agnostic-ai/ and run \`agnostic-ai sync\`.` Importer detection and `header.Strip` unchanged.
...
- `.aider.conf.yml` now carries the agnostic-ai provenance header so editors and grep-based detection treat it like every other generated file.
