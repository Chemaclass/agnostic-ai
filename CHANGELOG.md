...
- New `agnostic-ai why <file>` command. Reverse-provenance trace from an emitted file back to the adapter that wrote it, the source spec(s) that contributed (with `full` / `section` mode), the `outputs.<target>.*` config keys used to derive the path, and the last sync timestamp. Supports `--format json` for editor extensions and CI scripts. Closes #164.
- New `graph` command renders the spec → target → file dependency graph. `--format text` (default) prints an aligned matrix, `--format mermaid` and `--format dot` render diagrams, `--format json` is machine-readable. Filter with `--target`, `--spec`, `--kind`. Read-only: never invokes Emit on disk. Closes #172.
...
- `init` next-steps adapt to context: `--demo` / `--preset` advance straight to `sync`; detected CLIs surface as `import` follow-ups. Selected targets echoed back.
- `init` adds `.agnostic-ai/.sync-state` to `.gitignore` on scaffold. Closes #151.
- Provenance header shortened to `Generated by agnostic-ai` (drop `. Do not edit by hand.`). Detection marker unchanged — old files still recognized.
