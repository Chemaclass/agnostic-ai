...
- Continue MCP entries missing `command` on a stdio server or `url` on a remote one now emit no file and raise a coverage note, matching trae, antigravity and windsurf. Both fields are required, so the entry made Continue throw on the whole file (#739).
- Warp MCP entries missing `command` on a stdio server or `url` on a remote one are now skipped instead of written without the missing key. `.warp/.mcp.json` listed a server with nothing to launch, and the adapter's own doc comment claimed the opposite. The four-adapter list in the #739 entry above named warp for the same reason and has been corrected (#753).
