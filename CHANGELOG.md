...
- Every adapter output that supports comments now starts with a `Generated by agnostic-ai. Do not edit by hand.` header. Markdown files use an HTML comment placed below frontmatter when present; TOML/YAML use `#` line comments. JSON files (no comment syntax) stay unchanged. Importers strip the header so round-trips do not carry it back into source specs. Closes #140.
