...
### Added
- Codex adapter now emits `.codex/config.toml` with both `[[hooks.<event>]]` array-of-tables (lifecycle hooks: `SessionStart`, `Stop`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `PreCompact`, `PostCompact`) and `[mcp_servers.<name>]` tables. MCP stdio servers emit `command`/`args`/`env`; HTTP/SSE servers emit `url`/`bearer_token_env_var`/`http_headers`. Adds a shared `emit.WriteTOMLInlineStringTable` helper for emitting TOML inline tables like `env = { K = "v" }`. New `outputs.codex.mcp-file` config key (default `.codex/config.toml` — also holds hooks). README matrix upgrades Codex Hooks `-` to ✅ and MCPs `-` to ✅. Closes #78.
- Gemini adapter now emits `.gemini/settings.json` with both `mcpServers` and `hooks` maps. MCP stdio servers use `command`/`args`/`env`; HTTP/SSE servers use Gemini's `httpUrl` key (not the standard `url`) — the adapter handles the rename automatically. Hook specs route by their `event` frontmatter (e.g. `BeforeTool`, `AfterTool`, `SessionStart`) into `hooks.<event> = [{matcher, command}, ...]`. Pre-existing user-managed keys in `.gemini/settings.json` survive across syncs. New `outputs.gemini.mcp-file` config key (default `.gemini/settings.json` — also holds hooks). README matrix upgrades Gemini Hooks `-` to ✅ and MCPs `-` to ✅. Closes #79.
- Zed adapter now propagates MCP servers to `.zed/settings.json` under the `context_servers` key (Zed's own key — not `mcpServers`). Each server emits with Zed's nested `command: {path, args, env}` shape. Stdio is native; HTTP/SSE entries auto-bridge through `npx mcp-remote <url>` since Zed has no native HTTP transport. User-managed keys in `.zed/settings.json` (theme, font, etc.) are preserved across syncs. New `outputs.zed.mcp-file` config key (default `.zed/settings.json`). README matrix upgrades Zed MCPs from `-` to ✅. Closes #82.
- Amp adapter now propagates MCP servers to `.amp/settings.json` under the `amp.mcpServers` key. Stdio servers emit `command`/`args`/`env`; HTTP/SSE emit `url`/`headers`. Pre-existing non-managed keys in `.amp/settings.json` (theme, editor settings, etc.) are preserved across syncs; only `amp.mcpServers` is overwritten. Workspace MCPs require explicit approval inside Amp on first use — Amp's intentional safety model. New `outputs.amp.mcp-file` config key (default `.amp/settings.json`). README matrix upgrades Amp MCPs from `-` to ✅. Closes #81.
- OpenCode adapter now propagates MCP servers to `opencode.json` (project root) under the `mcp` key. Stdio servers map to `{type: "local", command: [cmd, args...]}`; HTTP/SSE/remote servers map to `{type: "remote", url, headers}`. Pre-existing non-managed keys in `opencode.json` (theme, model, etc.) are preserved across syncs; only `$schema` and `mcp` are overwritten. New `outputs.opencode.mcp-file` config key (default `opencode.json`). README matrix upgrades OpenCode MCPs from `-` to ✅. Closes #84.
- Continue adapter now propagates MCP servers to `.continue/mcpServers/<name>.yaml` (one YAML per server). Stdio servers emit `command`/`args`/`env`; HTTP / SSE / streamable-http emit `type`/`url`/`headers`. New `outputs.continue.mcp-dir` config key (default `.continue/mcpServers`). Plus new shared helper `emit.OutputMCPDir` and a `mcp-dir` field on `config.Output` for adapters that emit one MCP file per entry. README matrix upgrades Continue MCPs from `-` to ✅. Closes #80.
- Warp adapter now propagates MCP servers to `.warp/.mcp.json` using the standard `mcpServers` schema (stdio + HTTP/SSE transports). New `outputs.warp.mcp-file` config key (default `.warp/.mcp.json`). README matrix upgrades Warp MCPs from `-` to ✅. Closes #83.
- Amp adapter emits hierarchical `AGENTS.md` (root + per-scope subtree) plus per-agent slash commands at `.agents/commands/<name>.md` with `description:` frontmatter. Rules route by source-layout scope or `globs` prefix. The root `AGENTS.md` indexes agents/skills via pointer sections instead of duplicating bodies. Skills default to reference-only; flip `outputs.amp.emit-skills-as-commands: true` to also emit `.agents/commands/skill-<name>.md`. New config keys: `outputs.amp.commands-dir` (default `.agents/commands`), `outputs.amp.emit-skills-as-commands` (default `false`). README matrix upgrades Amp Agents/Skills from ◐ to ✅.
- New `emit.IsCapturing()` lets adapters skip filesystem side-effects (like the Amp rename migration) when running under capture mode (`sync --check`, drift inspection, `revert`).
- OpenCode adapter now emits per-agent slash commands at `.opencode/commands/<name>.md` with native OpenCode frontmatter (`description` required; optional `agent`, `model`, `subtask` from the `x-opencode` namespace). Skills default to reference-only listings in `.opencode/AGENTS.md`; flip `outputs.opencode.emit-skills-as-commands: true` to also emit `.opencode/commands/skill-<name>.md` per skill. The main AGENTS.md still merges rules but now references agents/skills via pointer sections instead of duplicating bodies. New config key: `outputs.opencode.commands-dir` (default `.opencode/commands`). README matrix upgrades OpenCode Agents from ◐ to ✅ (Skills stays ◐ — reference-only by default). Closes #66.
- Gemini adapter now emits hierarchical `GEMINI.md` plus TOML slash commands. Rules route by source-layout scope (e.g. `rules/backend/auth.md` → `backend/GEMINI.md`) or by a leading literal prefix of their `globs` frontmatter. Agents emit as `.gemini/commands/<name>.toml` per [Gemini CLI custom commands](https://geminicli.com/docs/cli/custom-commands/); the root `GEMINI.md` references them without duplicating bodies. Skills are reference-only by default; flip `outputs.gemini.emit-skills-as-commands: true` to also emit `.gemini/commands/skill-<name>.toml` per skill. New config keys: `outputs.gemini.commands-dir` (default `.gemini/commands`), `outputs.gemini.emit-skills-as-commands` (default `false`). README matrix upgrades Gemini Agents/Skills from ◐ to ✅. TOML primitives (`WriteTOMLString`, `WriteTOMLMultiline`, `WriteTOMLStringArray`, `EscapeTOMLBasic`) extracted from the Codex adapter into the shared emit package. Closes #65.
