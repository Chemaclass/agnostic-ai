...
- Warp MCP stdio entries now emit `working_directory` when a spec sets `cwd` (#606). `docs.warp.dev/agents/capabilities/mcp` documents it on the CLI Server (Command) table and warns to always set it when the command or args use relative paths; the shared MCP builder had no field for `cwd` at all, so it was dropped silently for warp and the seven other targets sharing that builder. `import warp` renames it back to `cwd` on the way in. claude, cursor, copilot, factory, junie, kiro, and qoder are unchanged: only warp's vendor doc confirms a destination key.
- The `target-audit` skill records four lessons from the 2026-08-09 run: address agents by spawn ID rather than bare name, since a long session accumulates same-named agents across runs and a stale one answered for the current batch; watch for shared-doc edits where **both** sides land intact, which git never flags and which shipped three contradictions in one run; expect fixers to correct the orchestrator's own prompt, since three of four did and all three were right; and try an existing escape hatch before filing something as a schema question, which is how a documentation gap was nearly booked as a spec change.
- Warp MCP entries no longer carry a `type` discriminant (#592). Warp's remote-server table documents only `url` and `headers`, and one tab covers "Streamable HTTP or SSE Server (URL)", so the transport is never named in config. The shared writer gained an `MCPSchemaServersMapNoType` variant for exactly this case; every other target keeps its `type` unchanged. `docs/user/targets.md` now lists warp on the correct side of that split, where it appeared on neither before.
- The capability tables in `docs/user/targets.md` and `README.md` no longer carry duplicated rows (#597). Parallel target-audit PRs had landed both sides of a change instead of merging them, leaving one trae row claiming no MCP surface next to one giving `.trae/mcp.json`, and one qoder row claiming no skills next to one giving `.qoder/skills/<name>/SKILL.md`. Each pair held one current and one stale row, and not in a consistent position, so the rows were collapsed per cell against the adapter source. A test now fails the build if a target appears twice in one table.
- `docs/user/spec-format.md` listed only Aider and Cline as having no project-scoped MCP file, while `targets.md` listed five (#597). Jules, Goose, and Augment were missing; the registry confirms the five.
- `docs/user/targets.md`'s `disabled` summary omitted Antigravity from the targets that honour it, contradicting `spec-format.md`'s own per-target table (#597).
- Documented that an Amp skill scopes its own MCP servers through `x-amp.mcpServers` (#591). Amp's manual recommends a skill-bundled server over user settings "for most use cases", and accepts them in `SKILL.md` frontmatter, which the existing passthrough already emitted verbatim. No spec change was needed: the audit assumed this required a new cross-kind relationship between an MCP entry and a skill, and it does not.
- All four `TestWatchSync_*` tests no longer flake on Windows CI (#585). The test edited a spec as soon as the initial sync's output file appeared, but the watcher takes its baseline mtime snapshot after that sync, so on a slow runner the edit landed before the baseline existed. The baseline then already contained it, no change was ever detected, and the test failed only when its own 15s deadline expired. They now wait for the watcher's banner, which prints immediately after the snapshot. The first fix covered only the one test that already captured output; `TestWatchSync_ReEmitsOnCodexOverlayChange` then failed the same way on the next PR, so all four now capture and wait rather than sleeping a fixed 80ms and hoping. All four watch tests also set mtimes explicitly with `os.Chtimes` rather than sleeping to advance them.
...
- `docs/user/spec-format.md` gains a `color` support-by-target table for Augment, Kilo Code, and Qoder, whose documented value spaces (ANSI name, hex/theme token, eight named colors) differ despite the shared key; `kilo.go` and `targets.md` no longer claim `color` matches Augment's own vocabulary, only that the key is shared and unvalidated. `targets.md` also documents zed's already-working `x-zed.disable-model-invocation` skill passthrough and records windsurf's un-written `.devin/mcp_config.local.json` MCP scope as a deliberate won't-fix (#609).
