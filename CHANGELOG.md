...
- Ignore specs reach Kiro (`.kiroignore`), Trae (`.trae/.ignore`), and Junie (`.aiignore`). All three document a native ignore file. Until now every ignore spec skipped on them with a warning, so paths a team declared off-limits stayed readable (#728).
- Gemini agents emit as native subagents at `.gemini/agents/<name>.md`. An agent is now delegatable, invokable with `@name`, and listed by `/agents`, instead of a slash command the user types. `tools` translates onto Gemini's own tool names. `import gemini` reads the directory back (#733).
- Trae hooks emit to `.trae/hooks.json` across six events, in the `{"version": 1, "hooks": {...}}` wrapper Copilot uses. A spec's `loop_limit` emits on `Stop`. A Claude-style matcher raises a coverage note: Trae spells the terminal tool `RunCommand`, not `Bash` (#729).
- Copilot hooks emit to `.github/hooks/agnostic-ai.json`, with an integer `{"version": 1, "hooks": {...}}` wrapper, `timeoutSec`, and 14 events (#629).
- Crush hooks emit to `crush.json`, merged alongside `mcp` in one write, `PreToolUse` only, matching the vendor's current support. `import crush` reads them back (#629).
- Factory hooks emit to `.factory/hooks.json` across nine events, keyed directly by event with no wrapper key. `timeout` is seconds, vendor default 60 (#629).
- Augment hooks emit to `.augment/settings.json`, merged alongside `mcpServers` in one write, across five events. `timeout` converts to milliseconds, since Augment's native unit differs from the spec's seconds. A `command` missing a `.sh`/`.ps1`/`.cmd`/`.bat` extension still emits and surfaces a coverage note, since Augment only runs a script path (#629).
- Hook specs accept `args`, which switches a Claude Code hook to exec form. The command spawns directly with no shell, so a path or argument carrying a space, apostrophe, `$`, or backtick runs as written. It survives `import claude` (#732).
...
- Copilot hooks emit `args` as Copilot's own `{"type": "command", "exec": ..., "args": [...]}`. The executable moves to `exec`, which is where Copilot's shape differs from Claude Code's. `exec` is Copilot CLI only, so a coverage note names the trade: a cloud agent job reads the same file and honors `bash` or `command` entries only. `args` previously reached no Copilot surface and was dropped in silence (#755).
- `lint` reports an MCP server missing the field its transport requires (LINT008, error): no `command:` on a stdio entry, no `url:` on an `http`, `sse`, or `ws` one. `validate` and `sync` both passed it. Trae, Antigravity and Windsurf dropped it in silence; Claude Code, Codex, Cursor, Gemini, Copilot and the rest wrote a server object with nothing to start or connect to (#747).

### Changed

- Gemini agents no longer write `.gemini/commands/<name>.toml` by default, and a managed file left there is swept. Set `outputs.gemini.emit-agents-as-commands: true` to keep the `/name` prompt. This also closes a collision where a same-named agent and command overwrote each other (#733).
- Qoder hook events sort in the vendor's own order. `TaskCreated`, `TaskCompleted`, `TeammateIdle`, and `Setup` used to trail any event Qoder does not document and now precede it. `.qoder/settings.json` changes bytes on the next sync for a project using one of those names alongside a custom event (#744).

### Fixed

- `sync` no longer silently destroys a hand-authored ignore file on cursor, gemini, aider, windsurf, kiro, trae, or junie. The first sync carrying any ignore spec replaced `.cursorignore` and its six siblings wholesale, deleting patterns whose whole job is keeping credentials out of agent context. No importer could recover them. `sync` now fails with `AAI-103` when the emitted patterns would drop something the file holds, names those patterns, and writes nothing. `import <target>` reads the file into an ignore spec, and the import summary counts ignores (#754).
...
- Continue MCP servers with `type: http` emit `type: streamable-http`, the only Streamable HTTP literal Continue's schema accepts. The previous value made Continue throw on the file, so the server never loaded (#726).
- Continue MCP `headers` emit under `requestOptions`, where Continue reads them. At the top level the schema stripped them and the server connected unauthenticated, failing later with a 401 and no sync-time warning (#730).
- Continue MCP servers with `type: ws` emit no file and raise a coverage note. Continue documents no websocket transport, and the name-only server written before matched neither branch of its schema (#726).
- Continue MCP `env` emits on stdio servers only, with a coverage note when a remote server declares one. Continue puts `env` on its stdio schema alone, so the map was stripped on load and the server ran without it (#739).
- Continue MCP entries missing `command` on a stdio server or `url` on a remote one emit no file and raise a coverage note, matching trae, antigravity and windsurf. Both fields are required, so the entry made Continue throw on the whole file (#739).
- Warp MCP entries missing `command` on a stdio server or `url` on a remote one are skipped instead of written without the missing key. `.warp/.mcp.json` listed a server with nothing to launch, and the adapter's own doc comment claimed the opposite. The adapter list in the #739 entry above named warp for the same reason and has been corrected (#753).
- Crush hooks accept every spelling Crush accepts. `pre_tool_use`, `pretooluse`, `PRETOOLUSE`, and `PRE_TOOL_USE` were dropped from `crush.json` and blamed on an unsupported event. All four now emit under the canonical `PreToolUse` key (#731).
- Cursor hook specs that filter `subagentStop`, `beforeSubmitPrompt`, `stop`, `afterAgentResponse`, or `afterAgentThought` no longer raise LINT005. Cursor documents all five as matcher-consuming, so the warning told users to delete a filter that works (#734).
...
- Docs no longer say Factory hooks merge into `.factory/hooks.json`. Every sync overwrites that file whole, so a hand edit is lost. This is the same correction #737 made for `.factory/mcp.json` on the neighboring surface. It matters on both because Factory's own docs send users to hand-edit them. No emitted bytes change (#745).
- Docs correct five stale vendor claims. Copilot's `subagentStart` and `userPromptTransformed` are camelCase-only, so a PascalCase spelling emits a key that never fires. Trae's Agents row shows `.trae/agents/<name>.md` instead of the pre-#638 flattened path. Zed's `timeout` works on either MCP transport, not HTTP alone. Qoder documents 27 hook events, not 23. `.factory/mcp.json` is overwritten each sync, not merged. No emitted bytes change (#737).
- Docs record that `.claude/settings.json` has become a cross-tool hook file, so syncing `claude` next to `copilot`, `cursor`, or `trae` can run the same hook twice. Copilot's read is on by default and has no toggle, and `claude` and `copilot` ship in the default target list together. Cursor's and Trae's each sit behind an off-by-default switch. No emitted bytes change (#755, #756).
...
- Audit sources gain cursor's uncited third-party-hooks page and its two enablement gates, trae's Claude Code hook read and the switch that guards it, copilot's combined-sources rule and cloud-agent field limits, and kilo's changelog head at v7.6.2 (#756).
...
- Qoder commands emit to `.qoder/commands/<name>.md`, with `description` frontmatter (required by the vendor; falls back to the command's name). Command specs previously reached no Qoder surface and skipped with a warning; `import qoder` now reads the directory back too (#630).

### Fixed

- Docs list every target that emits hooks and commands natively. The `spec-format.md` enumerations had drifted behind the adapters (hooks missed Kiro, Qoder, OpenHands and Zed's opt-in task mapping; commands missed Trae, Junie, Qoder and Kilo Code), and `targets.md`'s commands bullet missed Kilo Code (#629, #630).
...
- Kiro: skills now emit natively to `.kiro/skills/<name>/SKILL.md` instead of flattening into `.kiro/steering/skill-<name>.md`, keeping bundled `scripts/`, `references/`, and `assets/` and reaching Kiro's own skill picker; a stale flattened copy is swept. Hook `description` and `x-kiro` passthrough (e.g. `confirm`, Kiro's Stop-hook confirmation block) now reach `.kiro/hooks/<name>.json`, previously unreachable at any layer (#642).
