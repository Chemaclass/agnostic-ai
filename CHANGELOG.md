...
- Zed reads its entry point from `.rules` instead of the shared `AGENTS.md`. Zed takes the first match of an ordered lookup, and `.github/copilot-instructions.md` outranks `AGENTS.md` in it while carrying only the pointer body, so any project syncing copilot alongside zed silently lost every rule in Zed. `.rules` is first in that order, so Zed's rules no longer depend on which other targets are enabled.
- OpenCode reads its entry point from the root `AGENTS.md`, which it walks up the tree to find, instead of `.opencode/AGENTS.md`, which it never opens. An OpenCode-only project got no rules at all; a project also syncing codex, amp, warp, or zed worked only because those targets happened to write the shared root file. `import` still reads the old path when it is the only one present.
