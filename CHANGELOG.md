...
### Added
- `.agnostic-ai/AGNOSTIC_AI.md` is now tracked in git (removed from `.gitignore`). It is the canonical entry-point body that drives every per-target file (CLAUDE.md, AGENTS.md, GEMINI.md, ...); when present its content is propagated by `agnostic-ai sync`, so it belongs alongside the other source specs under `.agnostic-ai/`.
- `agnostic-ai sync` now records every emitted output path in a ledger inside `.agnostic-ai/.sync-state` (schema bumped to `version: 2`). On each subsequent sync the prior ledger is diffed against the current write set; any path the previous run wrote but the current run did not is removed via the header-guarded `RemoveGenerated` helper, and empty parent directories are pruned. Hand-authored files without the agnostic-ai provenance marker are preserved. This catches the general orphan class that previously needed adapter-specific sweeps (codex `.agents/` migration, removed spec files, target opt-outs, output-dir overrides) and works automatically for future renames. Older state files without the `outputs` field disable the sweep so the very first sync after upgrading just records a baseline.

### Changed
- The provenance header rendered into every generated file now reads `Generated by agnostic-ai. Do not edit this file directly; edit specs under .agnostic-ai/ and run \`agnostic-ai sync\`.` so users see the right edit target without leaving the file. The `Generated by agnostic-ai` substring is unchanged, so importer detection and `header.Strip` still work.
