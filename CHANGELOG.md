...
- `TestWatchSync_*` no longer flakes on Windows CI (#585). Four watch tests advanced a spec's modification time by sleeping and rewriting the file, but Windows updates last-write-time lazily, so the rewrite could land on the identical timestamp. The mtime poller then saw no change, the re-sync never fired, and the test failed only once its own wait deadline expired. They now set the timestamp explicitly with `os.Chtimes`, which removes the race rather than widening the window and drops the sleeps.
