...
- `target-audit` skill plus a `target-auditor` agent: a repeatable, parallel audit of all 25 adapters against their vendor's current docs, reporting evidence-backed drift (moved paths, new native surfaces, schema changes, deprecations) and optionally filing one issue per finding. Backed by `scripts/target-facts.sh`, which dumps a target's declared capabilities, default output paths, adapter package doc, and `docs/user/targets.md` rows in one call, and by a verified per-target list of vendor doc and changelog URLs.
