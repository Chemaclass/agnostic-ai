...
- Hook specs carry the current Claude command-hook schema: `async`, `asyncRewake`, `shell`, and `if` emit into `.claude/settings.json` and survive `import claude` round-trips; `commandWindows` emits into `.codex/hooks.json`.
