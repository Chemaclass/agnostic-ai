...
Keep each section under 400 words; the same test enforces it. A section holds the URL lines, what to watch, fetch quirks, and standing traps that once caused a wrong finding. Do not append dated "verified on" notes or release summaries that changed nothing we emit. Git history, `scripts/target-audit/signals.tsv`, and the closed `target-audit` issues keep that record.
...
- trap: `.windsurfignore` is not legacy. Only `.codeiumignore` is. `.devinignore` scopes indexing; "The agent additionally respects `.windsurfignore` files". One Ignore spec writes both; `outputs.windsurf.ignore-file` moves the indexing file only.
...
- status: **wound down.** continue.dev reads "Continue has joined Cursor". The docs still resolve and the adapter still emits. Treat the watch as low priority until the repo moves again.
...
- gap: the top-level `permission` key in `opencode.json` (`allow`/`ask`/`deny`) is unwritten and unnoted (#922). `/docs/policies/` is a different feature. Porting needs: `Edit` and `Write` collapse to `edit`; bare globs (`git *`) not `Bash(git:*)`; last-match-wins, so flatten into one ordered array.
...
- watch: MCP fields beyond stdio `command`/`args`/`env`/`cwd`, remote `serverUrl`/`headers`, and `disabled`. A fourth subagent `model` tier beyond `inherit`/`flash`/`pro`. Docs catching up on `inheritCustomizations`, the `rules:` key and `skills.json`/`agents.json`/`rules.json` discovery; they appear only in Hub changelog prose, so do not model them.
...
- trap: the triggers table lists 11 v1 triggers, all in `hookEventsByTarget["kiro"]`, plus Legacy Manual Hook, correctly absent. `Manual` looks ruled out, but that sentence covers IDE 0.x hooks. The CLI page lists it as current.
- trap: `AgentSpawn` was documented when #660 landed. 3.0 dropped it; it is not a misread.
- fact: hook `version` is the string `"v1"`, not the integer this adapter emits.
