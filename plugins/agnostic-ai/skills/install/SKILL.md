...
2. Detect the platform and pick the first route that applies:
   ...
   Check for a tool with `command -v brew` / `command -v npm` / `command -v go`.

   npm is published and works. It sits below the install scripts because npm 11 prompts before running install scripts, which pushes the binary download to first run. `npx agnostic-ai` runs the CLI with no install at all.

   winget and Scoop are wired into the release pipeline but not published yet, so `winget install Chemaclass.agnostic-ai` and `scoop install agnostic-ai` both fail today. Do not offer them.
