...
| Factory | Translated onto Factory's own fixed tool IDs: `Bash` becomes `Execute`, `Write` becomes `Create`, `WebFetch` becomes `FetchUrl`; `Read`, `LS`, `Grep`, `Glob`, `Edit`, and `WebSearch` carry over unchanged. DroidValidator rejects the file on any unknown ID, so a name outside that set is dropped and reported in a coverage note rather than written verbatim. Factory's `ApplyPatch` has no agnostic-ai counterpart; set `x-factory.tools` to reach it. |
