...
2. [Spec format](spec-format.md): the five kinds (agent, skill, rule, hook, MCP), per-directory scope, and the `x-<target>` namespace for tool-specific extensions.
3. [Targets](targets.md): capability matrix and per-target output paths (13 tools).
