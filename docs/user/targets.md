...
Rules with `scope` use native file conditions or nested instruction documents on 19 targets. Six targets skip them because native scope or its serialized format is unverified. See the [scope matrix and compatibility rules](scoped-context.md#native-support) before combining tools. The per-target layouts below describe ordinary rules unless a scoped case is stated.
...
- **Rules**: root rules inline into `AGENTS.md`. Scoped rules use `<scope>/AGENTS.md`; see [directory-specific instructions](scoped-context.md) for shared-reader compatibility.
...
Hooks reach five targets. Claude Code, Codex, Gemini, and Qoder all document the Claude-style `{"hooks": {"<Event>": [{"matcher", "hooks": [...]}]}}` shape at user scope, so one renderer serves them; Cursor keeps its own. The other seventeen are declined for a stated reason, not for lack of a surface: Factory keys `hooks.json` directly by event with no wrapper, Augment measures `timeout` in milliseconds and requires a script extension, Copilot uses `version: 1` with camelCase events and `timeoutSec`, Devin CLI keys `.devin/hooks.v1.json` as an unwrapped array of eight events with `type` accepting `prompt` as well as `command`, Antigravity nests events under a named hook object, Crush supports `PreToolUse` alone, Goose needs a wrapping plugin directory plus a manifest, Junie's are Early Access, Kiro uses a `{"version": "v1", "hooks": [...]}` array, and Amp, OpenCode, and Cline expose hooks only as TypeScript plugin modules. Adding those shapes is tracked in #629, which covers the same schemas at project tier.
