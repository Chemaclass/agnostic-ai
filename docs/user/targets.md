...
Targets sharing a path (codex, amp, warp, opencode, cline, junie, kiro, crush, trae, jules, goose, augment, qoder, openhands, factory, and kilo at `AGENTS.md`) write it once; dedup is automatic. Targets absent from the table above (cursor, windsurf, continue) have no root entry-point: they emit only per-file artifacts under their own directory.
...
```
AGENTS.md                                 # canonical entry-point pointer body (written by sync, shared)
