...
- **Hooks**: land in `.codex/hooks.json` (override via `outputs.codex.hooks-file`), routed by `event` frontmatter (`SessionStart`, `SubagentStart`, `UserPromptSubmit`, `PreToolUse`, `PermissionRequest`, `PostToolUse`, `PreCompact`, `PostCompact`, `Stop`, `SubagentStop`, `SessionEnd`) into per-event arrays with `matcher` and `command`. Optional `timeout`, `statusMessage`, `commandWindows`, and `additionalContextLimit` (token threshold for how much hook output reaches the model) pass through. The JSON form preserves matcher metadata the inline `[[hooks.<event>]]` TOML cannot.
...
- **MCP**: lands in `.codex/config.toml`. Servers emit as `[mcp_servers.<name>]`: stdio uses `command`/`args`/`env`/`cwd`, HTTP/SSE uses `url`/`bearer_token_env_var`/`http_headers`/`auth` (`oauth` or `chatgpt`). The project-tier config.toml is managed (overwritten each sync); put unmanaged Codex config in `~/.codex/config.toml`.
