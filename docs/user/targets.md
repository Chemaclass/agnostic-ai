...
| **windsurf**    | as `.md` rule (+ `.windsurf/workflows/<name>.md` w/ opt-in) | as `.md` (`skill-<name>.md`) | `.devin/rules/*.md`      | -     | - | - | - | - | - | - |
...
- **Hooks**: land in `.codex/hooks.json` (override via `outputs.codex.hooks-file`), routed by `event` frontmatter (`SessionStart`, `SubagentStart`, `UserPromptSubmit`, `PreToolUse`, `PermissionRequest`, `PostToolUse`, `PreCompact`, `PostCompact`, `Stop`, `SubagentStop`, `SessionEnd`) into per-event arrays with `matcher` and `command`. Optional `timeout`, `statusMessage`, and `commandWindows` pass through. The JSON form preserves matcher metadata the inline `[[hooks.<event>]]` TOML cannot.
...
Config keys: `outputs.windsurf.rules-dir` (default `.devin/rules`), `outputs.windsurf.workflows-dir` (default empty, opt-in).

Verify with the real IDE:

1. Install Devin Desktop from [devin.ai](https://devin.ai) (formerly windsurf.com).
2. Check the tree: `ls .devin/rules/`, `grep "Generated by agnostic-ai" .devin/rules/*.md` for the provenance header.
3. Open the project. Cascade loads every `.devin/rules/*.md`; each appears in the Rules panel with no "failed to parse" warnings.
