...
.mcp.json                                                  # only with outputs.copilot.root-mcp-file
```
...
- **MCP**: VS Code schema, top-level `servers` key, each entry carrying a `type` (`stdio`, `http`, or `sse`). VS Code's Agent Host does not read `.vscode/mcp.json` itself: [the vendor doc](https://code.visualstudio.com/docs/agent-customization/mcp-servers) says it "doesn't read `.vscode/mcp.json` directly" and that VS Code forwards the config instead, except servers needing interactive input. For a file Agent Host reads natively, set `outputs.copilot.root-mcp-file: .mcp.json`; the root copy carries the same `servers` wrapper, so the two files are byte-identical. It stays opt-in because a repository-root file is a surprise for a project that does not need it, and because agnostic-ai never emits `${input:...}` servers, so everything it writes already reaches Agent Host through the forward. One caveat: Claude Code writes its own `.mcp.json` at the root under `mcpServers`, not `servers`. Enabling both with this key pointed at the same path is a genuine conflict, and the sync collision check reports it rather than letting one silently overwrite the other.

Config keys: `outputs.copilot.instructions-dir` (default `.github/instructions`), `outputs.copilot.agents-dir` (default `.github/agents`), `outputs.copilot.skills-dir` (default `.github/skills`), `outputs.copilot.mcp-file` (default `.vscode/mcp.json`), `outputs.copilot.root-mcp-file` (unset, opt-in; mirrors the same servers to a workspace-root `.mcp.json`), `outputs.copilot.chatmodes-dir` (default empty, opt-in; emits one Custom Chat Mode per agent), `outputs.copilot.rules-file` (unset; writes always-on rules concatenated at that path and skips the pointer-body write).
