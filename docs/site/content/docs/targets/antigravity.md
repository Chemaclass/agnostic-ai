...
- **Rules**: every rule file starts with YAML frontmatter, its first bytes. "Every `.md` file inside `rules/` must start with YAML frontmatter declaring a valid `trigger`", and a file that omits it, or names an unrecognized value, "Antigravity silently discards" ([antigravity.google/docs/rules](https://antigravity.google/docs/rules)). A spec's `globs` maps onto `trigger: glob` plus a quoted, comma-joined `globs` value; `alwaysApply: false` with no `globs` maps onto `trigger: model_decision`, which the vendor's own table marks `description` required for, so a rule with neither falls back to `trigger: always_on` and reports a coverage note rather than writing a trigger the vendor calls invalid for want of that field; every other rule writes `trigger: always_on`. `description` carries through on any trigger when the spec has one. The provenance header lands after the closing `---`, never before it.
