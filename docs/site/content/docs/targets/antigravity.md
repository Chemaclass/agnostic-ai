...
`agnostic-ai import antigravity` prefers `.agents/rules` and `.agents/skills/`. It falls back to the singular `.agent/rules` and `.agent/skills/` only when the preferred directory is absent. Skill imports include all bundled assets. A rule's `trigger` frontmatter maps back onto `alwaysApply` (`always_on` to `true`, `glob`/`model_decision`/`manual` to `false`), `globs` carries through, and the singular `glob:` spelling folds onto the plural key; an unrecognized `trigger` value is left in place rather than guessed at.
