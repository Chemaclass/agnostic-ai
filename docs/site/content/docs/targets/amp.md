...
- **Settings**: merged into the same `.amp/settings.json`, which Amp documents as its [workspace settings file](https://ampcode.com/docs/cli/settings): "the nearest `.amp/settings.json` or `.amp/settings.jsonc`, searched upward from your current working directory to the repository root". Workspace settings override user settings. MCP servers and settings share one merge, so only `amp.mcpServers` and the keys written under `x-amp` are ever set and every other key in the file survives the sync.
  - **Amp is deny-only, and the portable lists do not translate yet.** [Amp's settings schema](https://ampcode.com/cli-settings.schema.json) declares twenty properties and none of them is a tool allow-list. `amp.tools.disable` is the only tool key: "Disable specific tools by name. Use `builtin:toolname` to disable only the built-in tool with that name while allowing an MCP server to provide a tool by the same name. Glob patterns using `*` are supported." The one allow-shaped construct, `amp.mcpPermissions`, matches MCP *servers* by `command` or `url`, not tools.
  - There is no `ask` tier either. [Amp's tools page](https://ampcode.com/docs/tools) states the opposite posture outright: "By default, Amp does not ask for approval before running tools", and its permissions section points at a [custom plugin](https://ampcode.com/docs/customize/plugins#example-plugin-permissions) rather than a setting. A portable `model` has no key here; Amp routes models through the Dial.
  - A portable `deny` has the right shape for `amp.tools.disable`, but Amp publishes no tool vocabulary: "You can see Amp's builtin tools by running `amp tools list` in the CLI". Translating `Bash(rm:*)` into a guessed name would write a file Amp silently ignores, so sync reports a coverage note instead. Write the deny-list yourself under `x-amp` with the names `amp tools list` prints, and the mapping lands once those names are confirmed (#950).
