...
Kiro's [documented agent schema](https://kiro.dev/docs/custom-agents/configuration-reference/) also carries `tools`, `mcpServers`, `permissions`, `hooks`, `keyboardShortcut`, `welcomeMessage`, `excludedTools`, `includeMcpJson`, and `includePowers`; only `tools` has an agnostic-ai spec equivalent. The rest reach the file through `x-kiro`.
