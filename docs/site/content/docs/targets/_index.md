...
- **Hooks**: shell commands on lifecycle events (`PreToolUse`, `PostToolUse`, `SessionStart`, and others). Event names, file paths, wrapper shapes, and timeout units all differ per target, so read the target's section before authoring one. Matchers fail more quietly: Claude Code's tool vocabulary reaches Qoder and Copilot's PascalCase form unchanged, but OpenHands, Windsurf, Augment, Crush, Factory, and Trae each name tools differently, so a Claude-authored matcher parses there and then matches nothing. Zed runs hooks only through the opt-in `outputs.zed.tasks-file`. Other targets skip with a warning.
  ...
  **Syncing `claude` alongside another target can run the same hook twice.** Copilot, Cursor, and Trae can read `.claude/settings.json` alongside their own hook files. Copilot loads it by default. Cursor's **Include Third-Party Plugins, Skills, and Other Configs** switch is also on by default; Trae's import switch requires opting in. See the [Cursor](@/docs/targets/cursor.md) and [Trae](@/docs/targets/trae.md) pages for their controls and event mappings. Review which files each enabled tool reads when scoping hook specs.
- **MCP servers**: propagate to every target with a project-scoped MCP file (21 of 25, see the matrix). Aider, Cline, Jules, and Goose have no MCP surface and skip with a warning. Transport fields follow each target's schema: Cursor requires `type: stdio`, while other targets can infer stdio from a command or use different transport names. Read the target page before hand-editing a server entry. Copilot emits twice, since Copilot CLI does not read VS Code's `.vscode/mcp.json`. See [`disabled` support by target](@/docs/spec-format.md#disabled-support-by-target) for which targets honor a spec's `disabled: true`.
