...
MCP servers merge into the `mcp` map of `kilo.jsonc`. Stdio combines `command`+`args` into one `command` array and sets `type: "local"`, using `environment` for env vars; remote sets `type: "remote"` and uses `url`/`headers`. A spec's `disabled: true` writes `"enabled": false`, the key Kilo Code's own documented MCP example carries; an enabled server gets no key at all. Both transports preserve `timeout` in milliseconds, including zero. Remote servers accept `oauth: false` to disable automatic OAuth; `x-kilo` can override both options.
