...
A settings spec's default `model` merges into the top level of `kilo.jsonc` alongside `instructions` and `mcp`. So does an `x-kilo` block on that spec, which is how a Kilo-only key such as `sandbox` reaches the file; `permission` is the exception, since it already merges tool by tool with the translated rules. `import kilo` restores that portable field to `settings/imported.yaml` and continues to import `.kilocodeignore`.
