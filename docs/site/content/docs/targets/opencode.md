...
- **Settings**: a settings spec's default `model` merges into the same `opencode.json`. Existing native keys survive. `import opencode` restores the field to `settings/imported.yaml`. An `x-opencode` block on a settings spec merges into that file too, for the keys this tool does not model; `permission` is the exception, since it already merges tool by tool with the translated rules.
