...
Hooks emit as a complete [Open Plugins](https://goose-docs.ai/docs/guides/context-engineering/hooks/) package. The manifest lives at `.agents/plugins/agnostic-ai/plugin.json`, and the wrapped hook map at `hooks/hooks.json`. All 12 documented events pass through, `matcher` is a regular expression, `timeout` is seconds, and `x-goose.on_failure` accepts `allow` or `block` for `PreToolUse` failure policy.
...
| `outputs.goose.skills-dir` | `.agents/skills` | |
