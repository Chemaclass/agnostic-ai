...
- **Review**: review specs emit as [Bugbot](https://cursor.com/docs/bugbot) files inside `.cursor/` directories: `.cursor/BUGBOT.md` at the repo root for unscoped specs, `<scope>/.cursor/BUGBOT.md` for scoped ones, with same-scope specs concatenated. Bugbot always includes the root file and picks up per-directory copies while traversing up from changed files. Override the basename via `outputs.cursor.review-file`. (#433)
  ...
  [Rule limits](https://cursor.com/docs/bugbot#rule-limits) treats a BUGBOT.md as one rule: "Each rule is truncated at 30,000 characters when included in a review. The combined rules Bugbot includes for a review are capped at 100,000 characters. If you exceed that combined cap, some rules may be omitted." agnostic-ai never truncates on the author's behalf, so an over-cap file still emits in full; `sync` raises a surface-gap note instead, measured on the emitted file (header included), so same-scope specs that are each small can still trip it once concatenated. Split the review across sibling scopes to stay under the per-file cap. One review reads the root file plus every BUGBOT.md on the way up from a changed file, so `sync` also notes any scope whose chain (the scope and its ancestors) passes the 100,000-character budget on its own. Team rules and repository rules count against that budget too and live outside this repo, so a chain under it can still lose rules; run `bugbot run verbose=true` or `cursor review verbose=true` on a pull request for Bugbot's own table of what it included, truncated, or omitted. (target-audit 2026-09-24, #1125)
