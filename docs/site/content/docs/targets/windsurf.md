...
- **Agents**: one custom subagent profile per agent at `.devin/agents/<name>.md`. Devin's docs call this "Custom subagents are defined as markdown files under `agents/`", project layout `.devin/agents/`, "**Flat file**: `agents/<name>.md`" ([docs.devin.ai/cli/subagents](https://docs.devin.ai/cli/subagents)). Frontmatter carries `name`, `description`, `model`, `allowed-tools`, and `max-nesting`; the body after the closing delimiter is the subagent's system prompt.
  ...
  - `allowed-tools` translates agnostic-ai's Claude-style names onto the five names Devin publishes: `read`, `edit`, `grep`, `glob`, `exec` ([docs.devin.ai/cli/reference/permissions](https://docs.devin.ai/cli/reference/permissions)). The subagent docs enumerate no vocabulary of their own, so this list is the only documented one for `allowed-tools`, and it is kept separate from the permission vocabulary below, which has already grown a sixth name. `Read`/`Grep`/`Glob`/`Bash` map one-to-one onto `read`/`grep`/`glob`/`exec`. `Write` and `Edit` both collapse onto `edit`, so an agent declaring only `Write` also gains edit capability. An `mcp__<server>__<tool>` name passes through untranslated. Anything else drops with a coverage note rather than shipping a name the vendor never documented.
