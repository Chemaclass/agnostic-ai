...
- **Agents**: one custom subagent profile per agent at `.devin/agents/<name>.md`. Devin's docs call this "Custom subagents are defined as markdown files under `agents/`", project layout `.devin/agents/`, "**Flat file**: `agents/<name>.md`" ([docs.devin.ai/cli/subagents](https://docs.devin.ai/cli/subagents)). Frontmatter carries `name`, `description`, `model`, `allowed-tools`, and `max-nesting`; the body after the closing delimiter is the subagent's system prompt.
  ...
  - `allowed-tools` translates agnostic-ai's Claude-style names onto Devin's own vocabulary, kept separate from the permission vocabulary below since the two pages draw from different vocabularies and have already moved apart once (#951). `/cli/reference/permissions` documents five names for `allowed-tools`, `read`, `edit`, `grep`, `glob`, `exec` ([docs.devin.ai/cli/reference/permissions](https://docs.devin.ai/cli/reference/permissions)), and lags its own changelog: CLI v3000.11.1 (September 21, 2026) adds a sixth, `write`, under `### Fixed`, "Custom subagent profiles and skills can grant the `write` tool using `allowed-tools`, and permission rules recognize it" ([docs.devin.ai/cli/changelog/stable.md](https://docs.devin.ai/cli/changelog/stable.md)). `Read`/`Grep`/`Glob`/`Bash` map one-to-one onto `read`/`grep`/`glob`/`exec`, and `Write`/`Edit` now map onto distinct Devin tools, `write` and `edit`, rather than both collapsing onto `edit` (#1022). An `mcp__<server>__<tool>` name passes through untranslated. Anything else drops with a coverage note rather than shipping a name the vendor never documented.
...
- **Scoped rules**: a scoped rule lands at `<scope>/.devin/rules/<name>.md`. This follows Desktop's documented discovery of "`.devin/rules` or `.windsurf/rules` in any sub-directory of your workspace" ([Desktop rules reference](https://docs.devin.ai/desktop/cascade/memories)). Since v3000.11.1 (September 21, 2026), Devin CLI also discovers rules recursively under `.devin/rules/` and `.windsurf/rules/` ([CLI changelog](https://docs.devin.ai/cli/changelog/stable.md)). This CLI change does not establish Desktop recursion or equivalent scope semantics for nested rule files. The output layout stays as adopted in #628 (2026-08-27); sync still sweeps the old `.devin/rules/<scope>/<name>.md` tree through the ledger.
...
- **Ignore**: ignore specs emit as both `.devinignore` and `.windsurfignore`, gitignore syntax under a `#` provenance header. "Paths matched by `.devinignore` are excluded from indexing and cannot be viewed, edited, or created by Devin" ([docs.devin.ai/desktop/context-awareness/devin-ignore](https://docs.devin.ai/desktop/context-awareness/devin-ignore)).
  - `.windsurfignore` and `.codeiumignore` are legacy names: "still read and enforced alongside `.devinignore`". Set `outputs.windsurf.ignore-file: .codeiumignore` to write that name instead of `.devinignore`.
  - The `.windsurfignore` copy covers older Windsurf builds, where it was the only file the agent honored for file access. Writing just the indexing file there put a user's `secrets/**` out of the index while the agent stayed free to read and edit it (#863). `outputs.windsurf.ignore-file` moves the main file only; `.windsurfignore` follows every time, unless the override names `.windsurfignore` itself.
...
- **Settings**: the portable `permissions` lists merge into `.devin/config.json` under `permissions`, the committed project policy Devin's own approval prompts write to: "Allow for project | `.devin/config.json` | Yes" ([docs.devin.ai/cli/reference/permissions](https://docs.devin.ai/cli/reference/permissions)). Only that key is set, so `read_config_from`, `hooks`, and any sibling inside `permissions` survive the sync. An `x-windsurf` block on a settings spec merges into the same file, so `read_config_from` and `hooks` are reachable from a spec; `permissions` is the exception, since it already merges per spec and per list with the translated rules. Those three are the only keys Devin accepts there: "Only `permissions`, `read_config_from`, and `hooks` are available in project configs" ([docs.devin.ai/cli/reference/configuration/config-file](https://docs.devin.ai/cli/reference/configuration/config-file)).
  ...
  - Devin has its own rule vocabulary, so each rule is translated rather than copied. `Read(glob)` and `Write(glob)` pass through, `Edit(glob)` collapses onto `Write(glob)`, `Bash(prefix:*)` becomes `Exec(prefix)`, `WebFetch(pattern)` becomes `Fetch(pattern)`, a bare tool name maps onto Devin's own (`Bash` to `exec`, and so on), and `mcp__<server>__<tool>` passes through untouched.
  - The bare-name table here is separate from the one `allowed-tools` uses. The permissions reference lists five names and lags its own changelog: `web_search` joined the accepted names in CLI v3000.10.21 (2026-09-10), "`web_search` can now be used as a tool name in `permissions.deny` / `permissions.ask` / `permissions.allow`; previously it was rejected and web searches were always auto-approved" ([docs.devin.ai/cli/changelog/stable.md](https://docs.devin.ai/cli/changelog/stable.md)). A portable `WebSearch` therefore becomes `web_search` here, and reads back the same way. A bare `WebFetch` still drops: `webfetch` appears in the lifecycle-hooks tool table and a user-tier `disabled_tools` example, neither of which governs `permissions`.
  - An exact `Bash(...)` rule with no `:*` suffix has no faithful form, because Devin's `Exec` only prefix-matches, so translating one always widens it. Which list holds the rule decides what happens next. On `allow` and `ask` the widening approves commands nobody listed, so the rule drops with a coverage note. On `deny` it blocks more than was asked for, which is the safe direction, so `Bash(rm -rf /)` is written as `Exec(rm -rf /)` rather than leaving the command unblocked. Devin ranks a command deny above a broader rule: "A command deny such as `Exec(rm)` blocks the command even when a broader `ask` or `allow` rule covers the whole tool" ([docs.devin.ai/cli/changelog/stable.md](https://docs.devin.ai/cli/changelog/stable.md), v3000.10.31). Set `x-windsurf.permissions` to write Devin's own rules directly; that object replaces the portable one for that spec and passes through untranslated.
...
`agnostic-ai import windsurf` reads rules from `outputs.windsurf.rules-dir` when set, the same path emission writes to; unconfigured, it falls back to `.devin/rules/`, then legacy `.windsurf/rules/`, and reclassifies each file by [filename prefix](@/docs/cli-reference.md#filename-prefix-reclassification). It also scans the whole project tree for a scoped copy of that directory (`<scope>/<rules-dir>/*.md`) to reconstruct a scoped rule. `CheckScopePath` rejects nothing about a name like `.github`, `vendor`, or `node_modules`, so a rule scoped there imports back too, not just a scope with a plain visible name (#1123).
