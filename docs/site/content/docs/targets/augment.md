...
Commands emit to `.augment/commands/`; a source-layout scope becomes a nested command namespace, while `description`, `argument-hint`, and `model` stay in frontmatter. Ignore specs emit to `.augmentignore`, and `import augment` restores a hand-authored file without changing pattern order or negation semantics.
