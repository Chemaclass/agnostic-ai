...
# `goose`, and `augment`, left out of the default set so enabling them is
# a deliberate choice. They still share the root `AGENTS.md` pointer body
# (written once via auto-dedup, since the body is byte-identical) when
# enabled alongside a target that also writes it. Enable them explicitly
...
outputs:
  ...
  gemini:
    agents-dir: .gemini/agents       # default. One native subagent .md per agent.
    commands-dir: .gemini/commands   # default. One .toml per command.
    ...
    emit-agents-as-commands: false   # default. When true, agents also emit the pre-v0.60 <name>.toml command.
  ...
  opencode:
    ...
    skills-dir: .opencode/skills         # default. One folder per skill (<name>/SKILL.md + bundled assets).
  ...
  junie:
    ...
    ignore-file: .aiignore              # default. Agent ignore patterns (gitignore syntax).
  kiro:
    ...
    skills-dir: .kiro/skills            # default. One folder per skill (<name>/SKILL.md), Kiro's own skill tree.
  ...
  trae:
    ...
    hooks-file: .trae/hooks.json        # default. version: 1 wrapper, six events, {matcher, hooks: [...]} groups.
    ignore-file: .trae/.ignore          # default. Indexing ignore patterns (gitignore syntax); applies after a re-index.
  qoder:
...
| `codex` | `mcp-file` | `.codex/config.toml` | Holds the portable default `model` and `[mcp_servers.<name>]` tables. Hooks moved out into `hooks-file`. |
...
| `gemini` | `agents-dir` | `.gemini/agents` | One native subagent `.md` per agent (`name` and `description` required; optional `kind`/`tools`/`model`/`temperature`/`max_turns`/`timeout_mins`). |
| `gemini` | `commands-dir` | `.gemini/commands` | One TOML per command. |
| `gemini` | `skills-dir` | `.gemini/skills` | One folder per skill (`<name>/SKILL.md` + bundled assets, the workspace tier Gemini CLI scans). |
| `gemini` | `emit-skills-as-commands` | `false` | When true, skills additionally emit as `.gemini/commands/skill-<name>.toml`. |
| `gemini` | `emit-agents-as-commands` | `false` | When true, agents additionally emit as `.gemini/commands/<name>.toml`, the only agent surface before agents moved to `.gemini/agents/`. Set it to keep typing `/name`. |
| `gemini` | `ignore-file` | `.geminiignore` | Agent ignore patterns, gitignore syntax. |
...
| `copilot` | `instructions-dir` | `.github/instructions` | One `.instructions.md` per rule. `applyTo:` frontmatter derived from `globs` or scope. |
| `copilot` | `agents-dir` | `.github/agents` | One `<name>.agent.md` custom-agent profile per agent (`name`, `description`, optional `tools`/`model`). |
...
| `opencode` | `agents-dir` | `.opencode/agents` | One native subagent `.md` per agent. Frontmatter filtered to `description`, `mode`, `model`, `temperature`, `permission`. |
| `opencode` | `skills-dir` | `.opencode/skills` | One folder per skill (`<name>/SKILL.md` + bundled assets). |
...
When omitted: the 20 default adapters above (every adapter except `amp`, `warp`, `jules`, `goose`, and `augment`, left out of the default set so enabling them is a deliberate choice). Enabling any of them alongside `codex` is safe: the identical shared-path body is written once via auto-dedup. Comment out entries to disable targets. CLI flag `-t/--target` overrides for a single run.
