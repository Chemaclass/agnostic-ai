...
**Why:** target users are AI tooling devs across stacks (JS, Python, Rust, Go); a PHP runtime cannot be assumed. Go ships a single static binary via Homebrew, `curl | sh`, or `go install` with zero runtime deps and trivial cross-compile (`GOOS GOARCH`). Phel showcase did not justify the reach loss. Tradeoff: lose Lisp dev velocity, gain ecosystem-agnostic distribution.
...
**Why:** matches Claude Code's native format; migration is copy-paste. Markdown body fits prompts. YAML frontmatter is widely understood (Jekyll, Hugo, MDX). Hooks use pure YAML: they have no body, only fields.
...
**Why:** target quirks must not leak across adapters. Adding a new adapter must not require touching existing ones. Premature abstraction across adapters has burned similar projects (compile-target frameworks, transpiler ecosystems).
