...
2. [Spec format](user/spec-format.md) — agents, skills, rules, hooks; `x-<target>` namespace.
3. [Targets](user/targets.md) — capability matrix and per-target output paths.
4. [Configuration](user/configuration.md) — `agnostic.config.yaml` schema.
5. [CLI reference](user/cli-reference.md) — every command and flag.
