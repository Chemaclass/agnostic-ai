...
agnostic-ai supports Claude Code, Codex, Gemini CLI, Cursor, GitHub Copilot, and [20 more targets](https://agnostic-ai.org/docs/targets/#capability-matrix). Support varies by spec kind. Gemini default-model settings and Qoder HTTP/prompt hooks emit natively. An agent's `effort` reaches Codex as `model_reasoning_effort`. Import preserves Cline and Continue rule activation and supported compatible skill roots. The target reference lists every capability, native path, and opt-in setting.
